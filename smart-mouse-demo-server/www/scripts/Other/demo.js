(function(){
	...
	var init_canvas = function(){
		...
		// setTimeout(function(){
		// 	var STEP = 10;
		// 	cubeWrapper.cube.updateMatrix();

		// 	var updateMatrix = cubeWrapper.cube.matrix.clone();
	...
	var handleRotation = function(data){
		...
		for(var i = 0; i < deltaThetas.length; i++){
			var deltaTheta = deltaThetas[i].deltaTheta * (Math.PI/180);
			...
			last_rotation_timestamp = deltaThetas[i].timestamp;
			setTimeout(function(){
				rotateCube(-1 * deltaTheta, -1 * deltaTheta);
			}, rotationTimeout);
		}
	};

	var rotateCube = function(roll, pitch){
		cubeWrapper.cube.rotation.z += roll;
		cubeWrapper.cube.rotation.x += pitch;
		cubeWrapper.cube.rotation.y += pitch;
