(function(){
	...
	var createCube = function(){
    	...
    	var cube = new THREE.Mesh(new THREE.CubeGeometry(100, 100, 300, 1, 1, 1, materials), new THREE.MeshFaceMaterial());
    	cube.overdraw = true;
    	scene.add(cube);
	...
	var init_canvas = function(){
		...
		//cubeWrapper.camera.position.y = 0;
		// setInterval(function(){
		// 	cubeWrapper.camera.position.z = cubeWrapper.camera.position.z - 5; 	
		// 	//cubeWrapper.camera.position.y += 50;
		// 	//cubeWrapper.renderer.setSize(300, 300);
		// 	//cubeWrapper.renderer.render(cubeWrapper.scene, cubeWrapper.camera);
		// }, 500);
		register_event_listeners();
	};

	var register_event_listeners = function(){
		addEvent(document, "click", _click_handler);
	};
	...
	var _click_handler = function(event){
		//OBJECT_SELECTED = !OBJECT_SELECTED ? true : false;
		//if(!OBJECT_SELECTED){
			console.log(cubeWrapper.cube.position.x);
			console.log(event.pageX);
			cubeWrapper.cube.position.x = event.pageX - 800;
			cubeWrapper.camera.position.x = event.pageX - 400;
			//cubeWrapper.cube.position.y = event.pageY;
			cubeWrapper.renderer.render(cubeWrapper.scene, cubeWrapper.camera);
		//}	
	}
	...
	var handleRotation = function(data){
		...
		for(var i = 0; i < deltaThetas.length; i++){
			var deltaThetaRoll = deltaThetas[i].deltaThetaRoll ? deltaThetas[i].deltaThetaRoll * (Math.PI/180) : 0;
			var deltaThetaPitch = deltaThetas[i].deltaThetaPitch ? deltaThetas[i].deltaThetaPitch * (Math.PI/180) : 0;
			var deltaThetaYaw = deltaThetas[i].deltaThetaYaw ? deltaThetas[i].deltaThetaYaw * (Math.PI/180) : 0;
	...
	var rotateCube = function(roll, pitch, yaw){
		cubeWrapper.cube.rotation.z += roll;
		//cubeWrapper.cube.rotation.x += pitch;
		//cubeWrapper.cube.rotation.y += yaw;
		//cubeWrapper.cube.position.x += 100;
		//cubeWrapper.cube.position.y += 100;
 		cubeWrapper.renderer.render(cubeWrapper.scene, cubeWrapper.camera);
	};
